...
A reusable AWS billing dashboard on Django 6.1, PostgreSQL 18, a durable collection queue and Caddy. The web application, database and a separate collector process run on one EC2 instance. The collector retains its exact customer-role allowlist; a host firewall blocks EC2 metadata access from containers and all host users except root and the collector. See [single-EC2 operations](docs/single-ec2-operations.md) for the current topology and its host-compromise limitation.

The customer → connection → account model supports single AWS accounts (including members), multiple payers, shared payers, effective ownership, projects, budgets and Alliance handoffs. Reporting uses decimal amounts, explicit currencies and inclusive UI/exclusive AWS dates. Six-hour polling does not imply six-hour AWS data freshness.

Onboarding has four dashboard stages: **Customer → AWS account → Connect account → Automatic data pull**. The customer administrator first creates one approved read-only IAM role using the generated trust and combined permission policies. An internal portfolio administrator with current MFA selects **Connect account** to authorize the exact role, verify AWS access and discover the selected account. The collection engine then automatically imports six completed months plus the current month of available billing history and any approved existing AWS budgets. Member budget readers import budgets only. Collection continues every six hours across all eligible connections; no first-pull click, GitHub action or EC2 sign-in is needed. **Sync & activity** shows progress, errors and the next pull. Individual and all-account pull controls remain available for an extra refresh within the administrator's access.

The activation service supports single AWS accounts and member budget readers with roles under `BillingConsole/`. Management/shared payers and existing roles with other paths retain the reviewed approval and allowlist workflow. No customer stack, agent, long-lived key or billing export setup is required. Optional capabilities and metadata need explicit approval. Trust verification checks the actual role trust document, exact collector principal, account identity and both negative External ID cases. See [customer role onboarding](docs/manual-iam-onboarding.md) and the [one-time activation service setup](deploy/onboarding-worker/README.md).
