...
Onboarding has four dashboard stages: **Customer → AWS account → Connect account → Pull initial data**. The customer administrator first creates one approved read-only IAM role using the generated trust and combined permission policies. An internal portfolio administrator with current MFA selects **Connect account** to authorize the exact role, verify AWS access and discover the selected account. Once it is ready, **Pull initial data** imports the available billing history and selected existing AWS budgets. The first successful pull enables automatic collection every six hours; existing accounts that already completed their initial import keep syncing. **Pull data for all connected accounts** on **Sync & activity** starts initial history for new, ready accounts and refreshes recent costs and budgets for existing accounts within the administrator's access.
