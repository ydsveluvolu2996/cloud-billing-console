> Historical console evidence below belongs to the earlier release and is not a real-customer reconciliation result for this build.
...
| reportName | Editable title and authorized personal/customer saved reports |
...
`parameters.normalize` validates HTML, saved reports and exports. `advanced_explorer.build_report` uses the existing daily account/service imports for simple reports. Other dimensions and advanced metrics produce parameter-specific `ExplorerQuery` records per customer. The separate collector service calls the allowed read-only AWS APIs, follows pagination, and atomically updates cached responses. Web requests never wait on AWS. First requests start within the worker's one-minute queue schedule; recently used reports and saved reports refresh on the six-hour schedule. AWS's billing publication delay still applies.

Cache identity includes customer, requester and current authorization fingerprint, connection/ownership version, role/external-ID fingerprint, operation and canonical parameters. Revocation invalidates old access immediately. Changing the chart style reuses the same data query. Active cache means opened within seven days; unused queries expire after 30 days. Saved report definitions persist and are resolved every six hours, including relative date ranges. Failed refreshes retain prior results with a visible warning. Missing customer responses produce an incomplete-report banner and block CSV export. Zero and missing amounts differ; negative credits and exact decimal amounts are preserved. Requests are processed in batches of 100 so large queues can span multiple minutes.
...
One manually created customer role identifies the exact collector principal and unique External ID. The minimum policy has `ce:GetCostAndUsage`, `ce:GetDimensionValues` and `iam:GetRole` on the exact role itself. Tags, categories, forecasts, resource detail, Organizations inventory and Budget import are separately approved optional capabilities. See [the reviewed policy map](manual-iam-onboarding.md); no customer stack update is required.

No customer software, access keys, administrator policy or workload permissions are required. Resource reports require AWS opt-in and supported lookback/service settings; tags must be approved and activated, categories approved, and forecasts require sufficient history. Missing optional capabilities remain visible and do not remove core stored reporting. The application does not activate paid granular features or modify customer billing preferences.
