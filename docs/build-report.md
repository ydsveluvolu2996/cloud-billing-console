...
Individual local MFA and configurable OIDC sit behind centralized customer/account authorization across pages, writes, bulk imports, reports, exports, history, background jobs and cache results. Actual PostgreSQL runtime roles enforce row/column privileges; guarded functions support invitations, offboarding and ownership restamping. An exclusion constraint rejects concurrent ownership conflicts. Bulk previews bind to immutable user IDs and current authorization fingerprints; both previews and query caches enforce the fingerprint in database policies, so revoked scopes or reused usernames cannot reopen old results. Existing histories and Alliance fields/notes/snapshots are retained, with append-only customer revision records added.
...
| `.venv/bin/python manage.py test billing.tests --noinput` (PostgreSQL final application suite) | **157 passed in 29.701 s**, no skips; actual restricted role, concurrency, isolation, MFA, OIDC, migration, reconciliation and retention tests included |
| `.venv/bin/python manage.py makemigrations --check --dry-run` | No model/migration drift |
| `.venv/bin/python manage.py migrate --noinput` | All migrations through 0020 applied to isolated synthetic database |
...
| `.venv/bin/python scripts/restore_drill.py --source billing_security_checks --target billing_security_restore_scoped_previews --bin-dir /opt/homebrew/opt/postgresql@17/bin --output docs/evidence/restore-drill.json` | 48 tables/568 synthetic records plus schema matched; 1.428 s, 185,004-byte dump; actual production backup/RTO not verified |
