...
CREATE POLICY web_scope ON billing_explorerquery TO billing_web USING (billing_customer_visible(customer_id) AND (requested_by_id::text=current_setting('billing.user_id',true) OR requested_by_id IS NULL)) WITH CHECK (billing_customer_visible(customer_id) AND (requested_by_id::text=current_setting('billing.user_id',true) OR requested_by_id IS NULL));
...
CREATE POLICY web_scope ON billing_bulkimport TO billing_web USING (uploaded_by=(SELECT username FROM auth_user WHERE id::text=current_setting('billing.user_id',true))) WITH CHECK (uploaded_by=(SELECT username FROM auth_user WHERE id::text=current_setting('billing.user_id',true)));
