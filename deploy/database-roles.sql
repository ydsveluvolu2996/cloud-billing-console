...
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=pg_catalog,public AS $$ BEGIN CASE tab
 ...
 WHEN 'billing_activationrequest' THEN RETURN (SELECT t1.customer_id FROM billing_activationrequest t0 JOIN billing_billingsource t1 ON t0.source_id=t1.id WHERE t0.id::text=rid);
...
ALTER TABLE billing_activationrequest ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS web_scope ON billing_activationrequest;
CREATE POLICY web_scope ON billing_activationrequest TO billing_web USING (billing_can_access(billing_parent_customer('billing_billingsource',source_id::text),NULL,false)) WITH CHECK (billing_can_access(billing_parent_customer('billing_billingsource',source_id::text),NULL,true));
DROP POLICY IF EXISTS collector_scope ON billing_activationrequest;
CREATE POLICY collector_scope ON billing_activationrequest TO billing_collector USING (true) WITH CHECK (true);
GRANT SELECT ON billing_activationrequest TO billing_web;
GRANT SELECT ON billing_activationrequest TO billing_collector;
