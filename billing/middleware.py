...
class SecurityMiddleware:
    ...
    def __call__(self, request):
        ...
        if request.user.is_authenticated:
            ...
            elif profile.external and not settings.EXTERNAL_PORTAL_ENABLED:
        ...
        with context(access), transaction.atomic():
            if connection.vendor == 'postgresql' and settings.DATABASE_RLS_ENABLED:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT set_config('billing.user_id', %s, true)", [str(access.user_id or '')])
